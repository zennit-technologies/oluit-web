...
function CheakoutPage() {
  ...
  useEffect(() => {
    if (couponCode) {
      if (couponCode.offer_type === "2") {
        ...
        // setDiscountCoupon(totalPrice - price);
        let discount = (parseInt(couponCode.discount)/100) * parseInt(totalPrice);
  ...
  const shippingHandler = (addressId, cityId) => {
    ...
    shippingRules &&
      shippingRules.map((f) => {
        if (parseInt(f.city_id) === cityId) {
          setShippingRulesByCityId((prev) => [...prev, f]);
        }
      });
  };
      ...
      if (selectedBilling && selectedShipping) {
        if (selectedRule) {
          if (selectPayment) {
            ...
            toast.error(
                ServeLangItem()?.Please_Select_Your_Payment_Method
            )
          }
        } else {
                  ...
                  {!newAddress && (
                    <div className="addresses-widget w-full">
                      <div className="sm:flex justify-between items-center w-full mb-5">
                        ...
                        >
                          <span className="text-sm font-semibold">{ServeLangItem()?.Add_New}</span>
                        </button>
                      </div>
                      {activeAddress === "billing" ? (
                        <div
                ...
                <div className="flex-1">
                  <div className="mb-10">
                    ...
                    <div className="discount-code  w-full mb-5 sm:mb-0 h-[50px] flex ">
                      ...
                      >
                        <span className="text-sm font-semibold">{ServeLangItem()?.Apply}</span>
                      </button>
                    </div>
                  </div>
                  <h1 className="sm:text-2xl text-xl text-qblack font-medium mt-5 mb-5">
